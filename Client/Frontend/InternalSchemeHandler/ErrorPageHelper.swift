...
class ErrorPageHandler: InternalSchemeResponse {
    ...
    func response(forRequest request: URLRequest) -> (URLResponse, Data)? {
        ...
        let tryAgain = Strings.ErrorPages.TryAgain
        ...
        if errDomain == kCFErrorDomainCFNetwork as String {
            ...
            if errCode == MozErrorDownloadsNotEnabled {
                let downloadInSafari = Strings.ErrorPages.OpenInSafari
            ...
            variables["warning_actions"] =
                "<p><a id='\(UserScriptManager.appIdToken)__firefox__visitOnce' href='#'>\(MessageCertVisitOnce)</button></p>"
        }
